...
class TRPO(Module):
    ...
    def act(self, state):
        ...
        state = FloatTensor(np.array(state).astype(np.float32))
    ...
    def train(self, env, render=False):
        ...
        for n in range(num_iters):
            ...
            print("Iterations: {},   Reward Mean: {}".format(n + 1, np.mean(rwd_iter)))
            ...
            #print('old_params1',old_params[0])
            old_distb = self.pi(obs)

            def L():
                distb = self.pi(obs)
                #print('distb',distb.loc)
                ...
                if use_baseline:
                    ...
                    return (disc * rets * torch.exp(
                                distb.log_prob(acts)
                                - old_distb.log_prob(acts).detach()
                            )).mean()#这里的rets实际上就是：使用Gt估计Q函数，在原文中是Q函数，这个近似就是reinforce，看网页https://sirlis.cn/posts/reinforcement-learning-Policy-Gradient/
            ...
            #print('kld',kld)
            #print('grad_kld_old_param',grad_kld_old_param.shape)
            ...
            s = conjugate_gradient(Hv, g).detach()###输入的是H和g，得到的是x，也即H^(-1)g
