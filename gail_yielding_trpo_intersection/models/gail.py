...
if not os.path.isdir(ckpt_path):
    os.mkdir(ckpt_path)
# 用于保存权重的子目录
ckpt_rlim_dir = os.path.join(ckpt_path, 'r_lim_trpo')
...
class GAIL(Module):
    ...
    def train(self, env, ckpt_path,render=False):
        ...
        for i in tqdm(range(1200)):  #tqdm(
            ...
            self.v.train()
            opt_v.zero_grad()
            loss_value = (-1) * gms * delta * self.v(obs).squeeze()#
            ...
            loss_d_list.append(loss.item())
            loss_v_list.append(loss_value.mean().item())
            L_pi_list.append(L().item())
        ...
        log_csv  = os.path.join(ckpt_path, "training_losses_trpo.csv")
